		...
		class Kirki_Installer_Section extends WP_Customize_Section {
			...
			protected function render() {
				...
				<li id="accordion-section-<?php echo esc_attr( $this->id ); ?>" class="<?php echo esc_attr( $classes ); ?>" style="border-top:none;border-bottom:1px solid #ddd;padding:7px 14px 16px 14px;text-align:right;">
					<?php if ( ! $installed ) : ?>
						<p style="text-align:left;margin-top:0;"><?php esc_attr_e( 'Please Install Kirki so that you can use this theme\'s features & customizations.', 'couture' ); ?></p>
						<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $plugin_install_url ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'couture' ); ?>" data-name="Kirki Toolkit">
							<?php esc_html_e( 'Install Now', 'couture' ); ?>
						</a>
					<?php else : ?>
						<p style="text-align:left;margin-top:0;"><?php esc_attr_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'couture' ); ?></p>
						<a class="install-now button-secondary button change-theme" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'couture' ); ?>" data-name="Kirki Toolkit">
							<?php esc_html_e( 'Activate Now', 'couture' ); ?>
						</a>
					<?php endif; ?>
				</li>
				<?php
